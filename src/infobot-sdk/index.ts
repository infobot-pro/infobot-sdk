...
export default class Infobot extends EventEmitter {
    ...
    constructor(
        ...
        public pingTimeout = 60_000,
        public pingInterval = 5_000,
        public pingEnable = false
    ) {
        super()
    ...
    private connect() {
        ...
        this.ws.on('open', () => {
            this.send(auth(this.config.appId, this.config.key))
            if (this.pingEnable) {
    ...
    private ping() {
        this.emit(WS_EVENTS.WS_PING)
    }

    private pong() {
        this.timeout()
        this.emit(WS_EVENTS.WS_PONG)
    }
